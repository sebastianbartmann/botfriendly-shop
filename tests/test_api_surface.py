...
@pytest.mark.asyncio
async def test_api_surface_openapi_json_with_application_json_scores_full(monkeypatch, fake_get_factory):
    check = APISurfaceCheck()
    monkeypatch.setattr(httpx.AsyncClient, "get", fake_get_factory({"https://example.com/api-docs": (404, "")}), raising=True)
    ...
    artifacts = {
        "openapi.json": {"status_code": 200, "text": "{}", "content_type": "application/json"},
    ...
    assert result.score == 1.0
    assert result.severity == Severity.PASS
    assert result.details["spec_found"]["/openapi.json"] is True
...
@pytest.mark.asyncio
async def test_api_surface_graphql_options_400_with_graphql_url_counts_as_partial(monkeypatch, fake_get_factory):
    check = APISurfaceCheck()
    monkeypatch.setattr(
        httpx.AsyncClient,
        "get",
    ...
    async def _fake_options(self, url, *args, **kwargs):
        return type("Resp", (), {"status_code": 400, "text": "", "url": "https://example.com/graphql"})()
    ...
    assert result.score == 0.5
    assert result.severity == Severity.PARTIAL
    assert result.details["graphql_options_status"] == 400
...
@pytest.mark.asyncio
async def test_api_surface_doc_links_with_api_or_developer_href_scores_partial(monkeypatch, fake_get_factory):
    check = APISurfaceCheck()
    monkeypatch.setattr(
        httpx.AsyncClient,
        "get",
    ...
    html = "<a href='/api'>API</a><a href='/developer'>Developer</a>"
    result = await check.run("https://example.com", {"index": {"status_code": 200, "text": html}})

    assert result.score == 0.5
    assert result.severity == Severity.PARTIAL
    assert "/api" in result.details["doc_links"]
    assert "/developer" in result.details["doc_links"]


@pytest.mark.asyncio
async def test_api_surface_api_docs_html_with_api_docs_final_url_counts(monkeypatch):
    check = APISurfaceCheck()
    ...
    async def _fake_get(self, url, *args, **kwargs):
        responses = {
            "https://example.com/openapi.json": (404, "", "application/json", "https://example.com/openapi.json"),
            "https://example.com/swagger.json": (404, "", "application/json", "https://example.com/swagger.json"),
            "https://example.com/api-docs": (200, "<html>docs</html>", "text/html", "https://example.com/api-docs"),
            "https://example.com/api/v1": (404, "", "application/json", "https://example.com/api/v1"),
            "https://example.com/api/v2": (404, "", "application/json", "https://example.com/api/v2"),
            "https://example.com/": (200, "<html></html>", "text/html", "https://example.com/"),
        }
    ...
    assert result.score == 1.0
    assert result.severity == Severity.PASS
    assert result.details["spec_found"]["/api-docs"] is True
